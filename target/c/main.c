...
Ts Yl0kc(Tu Anr); /* MIOModule.print */
...
/* IOModule done */
struct termios* Vorig_termios = 0;
...
void YQar(Tr A0, Tb A1, Tr A2) {
 ...
 switch (A0.type) {
  ...
  case 14:
   return;
  case 15:
   YP_P__Yalz__YwtA__YxaJa(A0.ptr,A1,ZconvertZioref(A2, YdXi__imtt, 0, 99999, 5238609)); return;
  case 16:
   return;
  case 17:
...
void YDKA() {
 ...
 sf.pos=2541100;
  ...
  tcgetattr(STDIN_FILENO, Vorig_termios);
...
int ZglobInit(int round) {
 ...
 if (round == 2001) {
   sf.pos=0;
   Vorig_termios = malloc(sizeof(struct termios));
 }
 topFrame = sf.prev;
 return done;
}
 ...
 */
void disableRawMode() {
  tcsetattr(STDIN_FILENO, TCSAFLUSH, Vorig_termios);
  free(Vorig_termios);
}
...
int Fmain(void) {
 ...
 YDKA();
 sf.pos=527001;
 Vchar = ((Ti (*)(void*))(Y59X.table[23]))(Y59X.ptr);
 sf.pos=527002;
 while (((Vchar != Y3QJ) && (Vchar != 113)))
 {
  sf.pos=527003;
  Yl0kc(Vchar);
  sf.pos=527004;
